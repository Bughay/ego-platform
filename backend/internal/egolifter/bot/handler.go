...
// Handler is the HTTP entry point for the EgoLifter bot.
type Handler struct {
	svc    *Service
	logger *slog.Logger
}

// NewHandler creates the EgoLifter bot HTTP handler.
func NewHandler(svc *Service, logger *slog.Logger) *Handler {
	return &Handler{svc: svc, logger: logger}
}

// RegisterRoutes attaches the bot endpoint to the mux, wrapped with the shared
// JWT middleware (mw).
func (h *Handler) RegisterRoutes(mux *http.ServeMux, mw func(http.Handler) http.Handler) {
	mux.Handle("POST /egolifter/chat", mw(http.HandlerFunc(h.Chat)))
	mux.Handle("GET /egolifter/chats", mw(http.HandlerFunc(h.ListChats)))
	mux.Handle("GET /egolifter/chats/{id}/messages", mw(http.HandlerFunc(h.ListMessages)))
	mux.Handle("DELETE /egolifter/chats/{id}", mw(http.HandlerFunc(h.DeleteChat)))
}
...
// Chat handles POST /egolifter/chat: one turn with the EgoLifter AI assistant.
func (h *Handler) Chat(w http.ResponseWriter, r *http.Request) {
	...
	if req.Message == "" {
		lib.WriteError(w, http.StatusBadRequest, "message is required")
		return
	}
...
// ListChats handles GET /egolifter/chats: all of the authenticated user's chats,
...
// ListMessages handles GET /egolifter/chats/{id}/messages: the full conversation of
// one of the authenticated user's chats, oldest message first.
...
// DeleteChat handles DELETE /egolifter/chats/{id}: removes one of the user's chats
// (and its messages, via the ON DELETE CASCADE foreign key).
