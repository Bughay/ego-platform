...
type Agent struct {
	...
	UserPrompt   string
	Memory       []Message
	Thinking     bool // run the model call as a Grok reasoning ("thinking") call
	Tools        []Tool
...
func (a *Agent) callModel(ctx context.Context, messages []Message) (string, []ToolCall, string, error) {
	...
	for i := 0; i < maxRetries; i++ {
		content, toolCalls, finish, err := grokToolCall(ctx, a.Model, messages, 0.1, a.MaxTokens, a.Tools, a.Thinking)
		if err == nil {
...
// standard Grok/OpenAI function-calling protocol end-to-end — no custom
